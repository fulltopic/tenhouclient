...
trait BaseScene {
  def addTran(tran: BaseTransaction): Unit;
  def getTrans(): List[BaseTransaction];

  def createTran(): BaseTransaction
...
//  def setRonIndicator(who: Int, ronType: Int): Unit;
  ...
  def setAgari(who: Int, fromWho: Int, reward: Int, tiles: Array[Int]): Unit;
  def setRyu(reward: Int, tiles: Array[Int]): Unit;
